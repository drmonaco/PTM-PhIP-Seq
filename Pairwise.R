...
input = list()
input[[1]] = "~/Desktop/PAD51.csv" # path to phip RC data
input[[2]] = "~/Desktop/pad_51_key.csv" # path to table listing pairs of pairwise data
#input[[3]] = 1
input[[4]] = 100 # number of bins to make
input[[5]] = "~/Desktop/"# Directory of stored data
input[[6]] = "x" # analyze data on per library level or all librareis
#input[[7]] = 7 

if(!dir.exists(input[[5]])){
  dir.create(input[[5]])
}
...
input2 = input2[,c(2,1)]
tweak = input[[3]] %>% as.numeric()
bins = input[[4]] %>% as.numeric()
order = rownames(Utah)
out_path = input[[5]]
...
for(R in 1:dim(input2)[1]){
  ...
  full_plot[,6] = "black"
  full_plot[full_plot[,3]>=input[[7]],6] = "red"
  full_plot[full_plot[,4]>=input[[7]],6] = "blue"
  ...
  if (input[[6]]=="yes"){
    ...
    b = ggplot(full_plot)+geom_point(aes(x = full_plot[,1]+1,y = full_plot[,2]+1,color = full_plot[,6],size = .5))+
      scale_x_log10()+scale_y_log10()  + scale_colour_manual(values=c("black","red","blue"))+
      theme(legend.position = "none")+xlab(as.character(pair[2]))+ylab(as.character(pair[1]))
    ggsave(paste0(out_path,"/",pair_name_1,".jpg"),b,width = 15,height = 15)
